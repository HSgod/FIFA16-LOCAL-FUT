...
APP_NAME = "FIFA16 Local FUT"
VERSION = "1.0.0-fifa16-release"
...
# %%LOCALAPPDATA%%\FIFA16LocalFUT.
_local_app_data = os.environ.get("LOCALAPPDATA")
if _local_app_data:
    RUNTIME_ROOT = Path(_local_app_data) / "FIFA16LocalFUT"
else:
    RUNTIME_ROOT = Path.home() / "AppData" / "Local" / "FIFA16LocalFUT"
...
LEGACY_DB_PATH = DATA / "localfut16.sqlite3"
DB_PATH = RUNTIME_ROOT / "fut16-local.sqlite3"
...
class State:
    ...
    def _init(self) -> None:
        with self.lock:
            ...
            self.set_default("sid", "LOCAL16-" + sha1_text(str(time.time()))[:20].upper())
            self.set_default("token", "LOCAL16TOKEN-" + sha1_text(str(time.time() + 1))[:24].upper())
...
def route_fut(method: str, raw_path: str, headers: dict[str, str], body: bytes) -> tuple[int, dict[str, str], bytes]:
    ...
    if '/ut/game/fifa15/' in low:
        low = low.replace('/ut/game/fifa15/', '/ut/game/fifa16/')
    if '/pow/store/game/fifa15/' in low:
        low = low.replace('/pow/store/game/fifa15/', '/pow/store/game/fifa16/')
    ...
    if low.startswith("/pow/"):
        ...
        if low == "/pow/store/game/fifa16/catalog/list":
            return 200, {}, json_bytes({
                "catalogs": [
    ...
    if low == "/ut/game/fifa16/user/accountinfo":
        return 200, {}, json_bytes({
            "userAccountInfo": {
                "personas": [{
                    "personaId": int(CFG["persona_id"]),
    ...
    if low == "/ut/game/fifa16/user/club" and method in ("PUT", "POST"):
    ...
    if low == "/ut/game/fifa16/settings":
    ...
    if low.startswith("/ut/game/fifa16/phishing/trusteddevice"):
        return 200, {}, json_bytes({
    ...
    if low == "/ut/game/fifa16/match/reset":
    ...
    if low == "/ut/game/fifa16/match/end":
        doc = payload if isinstance(payload, dict) else {}
    ...
    if low == "/ut/game/fifa16/squad" and method == "GET":
        return 200, {"Cache-Control": "no-store"}, json_bytes(_native_squad_list())

    if low == "/ut/game/fifa16/squad" and method == "POST":
    ...
    if low == "/ut/game/fifa16/squad/clone" and method == "POST":
        src = dict(payload) if isinstance(payload, dict) else {}
    ...
    squad_id_match = re.match(r"^/ut/game/fifa16/squad/(\d+)$", low)
    if squad_id_match:
    ...
    if low == "/ut/game/fifa16/eventfeed":
        return 200, {}, json_bytes({"event": [], "timestamp": now_s()})

    if low == "/ut/game/fifa16/hub":
    ...
    if low == "/ut/game/fifa16/clubuser":
    ...
    if low == "/ut/game/fifa16/leaderboards/options":
        return 200, {}, json_bytes({})

    if low.startswith("/ut/game/fifa16/club/consumables"):
    ...
    if low == "/ut/game/fifa16/club/stats/consumables":
        return 200, {"Cache-Control": "no-store"}, json_bytes(_consumable_stats())

    if low.startswith("/ut/game/fifa16/club/stats/"):
        return 200, {}, json_bytes(_club_stats())

    if low == "/ut/game/fifa16/club":
    ...
    m_identity = re.fullmatch(r"/ut/game/fifa16/item/(\d+)", low)
    ...
    if low == "/ut/game/fifa16/user/credits":
        return 200, {}, json_bytes(_credits_payload())

    if low in ("/ut/game/fifa16/store/purchasegroup/all", "/ut/game/fifa16/store/purchasegroup/cardpack"):
    ...
    if low == "/ut/game/fifa16/purchased/items":
        if method == "POST":
    ...
    if low == "/ut/game/fifa16/watchlist":
    ...
    if low == "/ut/game/fifa16/transfermarket" and method == "GET":
    ...
    if low in (
        "/ut/game/fifa16/tradepile/counts",
        "/ut/game/fifa16/auctionhouse/counts",
    ):
    ...
    if low in (
        "/ut/game/fifa16/auctionhouse/pricelimits",
        "/ut/game/fifa16/marketdata/pricelimits",
        "/ut/game/fifa16/marketdata/item/pricelimits",
    ):
    ...
    if low == "/ut/game/fifa16/auctionhouse" and method == "POST":
        src = payload if isinstance(payload, dict) else {}
    ...
    native_offer = re.match(r"^/ut/game/fifa16/trade/(\d+)/(?:offer|bid)$", low)
    auction_bid = re.match(r"^/ut/game/fifa16/auctionhouse/(\d+)/bid$", low)
    ...
    if low in ("/ut/game/fifa16/auctionhouse/status", "/ut/game/fifa16/trade/status"):
    ...
    if low == "/ut/game/fifa16/auctionhouse/relist" and method in ("PUT", "POST"):
    ...
    if low == "/ut/game/fifa16/auctionhouse/sold" and method in ("DELETE", "POST"):
        removed = 0
    ...
    if low == "/ut/game/fifa16/tradepile":
    ...
    if bool(CFG.get("offline_seasons_enabled", True)) and low == "/ut/game/fifa16/season":
    ...
    if bool(CFG.get("offline_seasons_enabled", True)) and low == "/ut/game/fifa16/season/list":
        season_type = str(query.get("type", ["offline"])[0] or "offline").lower()
        if season_type == "offline":
            all_defs = _offline_season_definitions()
    ...
    if bool(CFG.get("offline_seasons_enabled", True)) and low == "/ut/game/fifa16/season/user":
    ...
    season_progress = re.match(r"^/ut/game/fifa16/season/(\d+)/division/(\d+)/user$", low)
    ...
    if bool(CFG.get("offline_seasons_enabled", True)) and low == "/ut/game/fifa16/season/user/history":
    ...
    season_reset = re.match(r"^/ut/game/fifa16/season/(offline|online)/reset$", low)
    ...
    season_quit = re.match(r"^/ut/game/fifa16/season/(\d+)/division/(\d+)$", low)
    ...
    apply_resource = re.match(r"^/ut/game/fifa16/item/resource/(\d+)$", low)
    apply_item = re.match(r"^/ut/game/fifa16/item/(\d+)$", low)
    ...
    # Single item:
    #   DELETE /ut/game/fifa16/item/<itemId>
    # Multiple items ("Quick Sell All"):
    #   POST /ut/delete/game/fifa16/item
    #
    # FIFA expects totalCredits + items[] in the response.
    single_discard = re.match(r"^/ut/game/fifa16/item/(\d+)$", low)
    ...
    if low == "/ut/delete/game/fifa16/item" and method == "POST":
        ids = []
        if isinstance(payload, dict):
    ...
    if low == "/ut/game/fifa16/item" and method in ("PUT", "POST"):
        results = []
        duplicate_results = []
    ...
    return 200, {"X-LocalFUT16-Unknown": "1"}, json_bytes({})


class FutHandler(BaseHTTPRequestHandler):
    protocol_version = "HTTP/1.1"
    server_version = "LocalFUT16/0.1"
...
class RedirectHandler(BaseHTTPRequestHandler):
    protocol_version = "HTTP/1.1"
    server_version = "LocalFUT16Redirector/0.1"
    ...
    def do_POST(self) -> None:
        ...
        if urllib.parse.urlsplit(self.path).path.lower() == "/extension":
            self._send(
                ...
                "text/xml",
                {
                    "EASW-Session": "fut16-local-easw-session",
                    "EASW-Token": "fut16-local-easw-token",
                },
            )
        else:
...
class DualModeTLSServer(ThreadingHTTPServer):
    """ThreadingHTTPServer that dynamically serves both plaintext HTTP and TLS ClientHello."""
    tls_ctx: Any = None

    def __init__(self, server_address: tuple[str, int], RequestHandlerClass: Any, bind_and_activate: bool = True):
        super().__init__(server_address, RequestHandlerClass, bind_and_activate)
        self._init_tls()

    def _init_tls(self) -> None:
        cert_candidates = [
            ROOT / "tls" / "winter15-chain.pem",
            ROOT / "winter15-chain.pem",
            ROOT.parent / "tls" / "winter15-chain.pem",
            Path.home() / ".localfut16" / "winter15-chain.pem",
        ]
        key_candidates = [
            ROOT / "tls" / "winter15.key",
            ROOT / "winter15.key",
            ROOT.parent / "tls" / "winter15.key",
            Path.home() / ".localfut16" / "winter15.key",
        ]
        cert_file = next((c for c in cert_candidates if c.is_file()), None)
        key_file = next((k for k in key_candidates if k.is_file()), None)
        if cert_file and key_file:
            try:
                ...
                self.tls_ctx = ctx
                log.info("Redirector TLS loaded: %s", cert_file.name)
            except Exception as exc:
                log.warning("Could not initialize Redirector TLS: %s", exc)

    def get_request(self) -> tuple[Any, Any]:
        sock, addr = self.socket.accept()
        if self.tls_ctx is not None:
            try:
                # Peek first byte: 0x16 is TLS Handshake ClientHello
                ...
                log.warning("Redirector TLS handshake error from %s: %s", addr, exc)
                return sock, addr
        return sock, addr
...
def main() -> int:
    ...
    try:
        ...
        redir = _bind_service(host, "redirect_port", "Redirector", DualModeTLSServer, RedirectHandler)
