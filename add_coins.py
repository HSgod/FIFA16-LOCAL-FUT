...
def main() -> int:
    ...
    if local:
        root = Path(local) / "FIFA16LocalFUT"
    else:
        root = Path.home() / "AppData" / "Local" / "FIFA16LocalFUT"
        
    root.mkdir(parents=True, exist_ok=True)
    db = root / "fut16-local.sqlite3"
    ...
    print("If FIFA 16 / FUT is open, leave FUT and re-enter (or restart Local FUT) to refresh balance.")
    return 0
